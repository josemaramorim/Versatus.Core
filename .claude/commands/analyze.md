---
description: Etapa 5 do SDD (gate duro) — cruza constitution ↔ spec ↔ plan ↔ tasks ↔ legado ↔ Matriz RTV ↔ Matriz ROT e reprova o módulo se alguma regra/validação/operação legada ficar órfã.
argument-hint: MOD-XX (ex. MOD-05)
---

Módulo alvo: `$ARGUMENTS`

Pré-condição: `spec.md`, `clarify.md`, `plan.md` (+ `research.md`, `data-model.md`,
`contracts/`), `tasks.md`, `matriz-rtv.md`, `matriz-rot.md` existem.

Dispare a skill **sdd-analyze** e siga `.agents/skills/sdd-analyze/SKILL.md` na íntegra.

Execute todas as verificações V1–V7:
...
Gere `specs/modulos/MOD-XX/analyze-report.md` com resumo, achados (severidade), órfãs por
categoria e **veredito**. Não corrija — aponte a ação exigida. `/implement` é proibido
enquanto o veredito for ⛔. Commit `docs(mod-XX): relatório de análise de cobertura (SDD
etapa 5)`.
