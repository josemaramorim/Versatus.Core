...
| **MOD-05** | ✅ Concluído | Estrangulamento (MOD-05) | `feat/estrangulamento-documento-financeiro` | Substituídas consultas locais por chamadas HTTP no legado usando `Servidor.Strangler`. |
...
- **2026-06-12 (GestaoFinanceira):** Concluído o estrangulamento de validações e parâmetros das entidades `Documento` e `DocumentoFinanceiroBase` no projeto legado. Criado o projeto `Servidor.Strangler` sob a estrutura de pastas recomendada. Commits adicionados na branch local `feat/estrangulamento-documento-financeiro`.
