...
| **MOD-02 (Parametro)** | ✅ Concluído | Migração FParametro | `feat/migrate-parametro` | Migrado formulário Parametro (C# e React) com suporte a paginação, filtros e controle de sequenciais. |

### Histórico Recente de Decisões:
- **2026-07-20 (AcessoGlobal):** Migrado o formulário legado `Parametro` e valores `ParametroValor` para a arquitetura .NET Core + React. Implementados DTOs, serviço com geração de sequenciais, controllers, form types, schema Zod, e configuração de tela com bloqueio de edição da Chave. Commits adicionados na branch `feat/migrate-parametro`.
