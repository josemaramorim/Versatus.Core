...
| **MOD-05** | 🔄 Implementação iniciada (SDD etapa 6) | Fase S — `S-T01` ✅; próxima `S-T02` | `develop` (nenhuma `feat/` aberta) | Camada SDD **mesclada em `develop`** (PR #1). `S-T01` mesclada em `develop` (PR #3, `df9a4ee`): projeto `src/Versatus.SharedKernel/` (`net10.0`, ref. `Versatus.Framework`, sem pacotes NuGet ainda) + entrada no `Versatus.Core.slnx`; `dotnet build` 0/0. Pré-requisitos pendentes: MOD-02 migrar rateio antes de E5 (CLR-01); dump de schema de produção antes de E2/E11. |
...
- **MOD-05:** implementação em andamento pelo `/implement`. `S-T01` ✅ (em `develop`). Próxima: **`/implement MOD-05 S-T02`** — criar `Versatus.GestaoFinanceira` + estrutura de pastas (`plan.md §1.2`); depois `S-T03` (DbContexts Write/Read + `RepositorioBase`) e `S-T04` (projeto de testes + DI + registro na WebAPI). Em seguida a Fase E0 (`Versatus.SharedKernel` — enums e abstrações). Pré-requisito antes do épico E5: migrar `RateioMovto`/`RateioMovtoItem`/`ManutencaoRateio` no MOD-02 (CLR-01). Antes de E2/E11: obter dump de schema de produção das tabelas ausentes no banco de dev (`FINPROJECAOFLUXOCAIXA*`, `FINLOGDOMINIOPERIODO`). Gate: tarefas `domain`/`service`/`operation`/`frontend` de cada épico só entram em `develop` após `/analyze MOD-05 --epico E?` ✅.
...
> **Para retomar o MOD-05:** etapas SDD 1-5 concluídas e em `develop`. A implementação vai pelo comando `/implement MOD-XX <TaskID>` (etapa 6) — próxima tarefa: `/implement MOD-05 S-T02`. O gate `/analyze MOD-05 --epico E?` roda por épico assim que a tarefa `analysis` (`E?-T01`) gera as matrizes RTV/ROT; `Z-T02` é a consolidação final (V1–V7 + cross-épico) antes de fechar o módulo.
